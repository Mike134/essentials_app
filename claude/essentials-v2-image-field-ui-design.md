...
**Status: DONE, 2026-09-03, real-device verified on MIKE-CU and MIKE-12R, both directions, through several full rounds (capture, drop, remove, recapture).** All six build order steps below shipped and confirmed working end to end: captured a real photo on MIKE-12R (Camera button), confirmed it appeared on MIKE-CU (Windows app, pulled via `FileSyncService.fetch`) -- byte-identical file confirmed on both the hub's canonical store and MIKE-CU's local cache, `6955388` bytes, `diff` clean. Later rounds (drag-and-drop on MIKE-CU, remove-then-recapture on both devices) confirmed the same both directions.

Real-device testing surfaced several genuine bugs beyond the one hub-side sync hiccup (`claude/essentials-v2-architecture.md`'s "Known Systemic Risks," occurrence 6, recovered with the existing `tool/adopt_migrations.dart`) -- all fixed, see CLAUDE.md's "Image field: designed, built, real-device verified" session write-up for the full list: Manage Fields' FAB covering the last field's drag handle, the Windows drop target having no resting-state visual cue, CSV export silently dropping the field entirely, and -- the most serious -- a recaptured image never displacing a stale copy already cached on another device (fixed by timestamping every capture's filename instead of reusing a fixed one, see the storage design doc's "Content-addressing" entry for the full writeup). Delete behavior (one of the storage design doc's originally-open items) also resolved: real file deletion on both "Remove" and record delete, plus a "Save..." button to rescue a copy first.
