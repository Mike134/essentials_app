...
- **`Future<void> upload(...)`** — called right after the local write at capture/drop time (per the `image` field design's "write locally first, offline-safe" step). `PUT`s the bytes to the hub via `SyncService.resolveServerAddress`. On failure (hub unreachable), swallow and log, same as `fetchFromServer()`.

  **Correction, 2026-09-03, while actually implementing this:** an earlier draft of this section claimed "the next successful `SyncService` reconnect is the natural retry point" — that's not actually true for `upload`, and saying so here overstated what the UI design doc (`claude/essentials-v2-image-field-ui-design.md`) deliberately chose: `FileSyncService` is **not** wired into `SyncService`'s connect/reconnect lifecycle at all (unlike `MigrationService.fetchFromServer`, which genuinely does re-run on every reconnect). A failed upload has **no retry mechanism whatsoever** in v1 — if the hub is unreachable at capture time, the image stays local-only on the capturing device until something else re-triggers an upload for that same field. Documented as a known, accepted gap on `FileSyncService.upload`'s own doc comment, not silently glossed over — worth a real retry/backoff story if offline capture turns out to bite in practice.
...
- ~~**No delete route.**~~ — **DONE, 2026-09-03.** `DELETE /files/{table}/{record_id}/{field_name}/{filename}` -- idempotent (always `200`, whether or not the file existed), never touches parent directories. Verified against an isolated scratch harness: existing-file delete, a follow-up `HEAD` confirming it's genuinely gone (not just trusting the `200`), idempotent re-delete, and delete-of-something-that-never-existed, all correct. Client-side trigger is `FileSyncService.delete`/`deleteByRelativeKey` -- see the image field design doc's "Delete behavior" entry for the two call sites.
