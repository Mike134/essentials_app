...
- ~~**Delete behavior**~~ — **resolved and DONE, 2026-09-03.** Real deletion (not a tombstone), on both the "Remove" button (one field's value) and record deletion (`GenericListScreen`'s own confirmation already says "This cannot be undone," so eager cleanup matches what's already promised). `GenericDao.delete` and `GenericFormScreen._clearImageField` both call `FileSyncService.deleteByRelativeKey`, which deletes the local file and calls the hub's new `DELETE /files/...` route. Fire-and-forget on the hub side, same no-retry posture as `upload` -- a delete that can't reach the hub leaves that copy orphaned, not a regression from the concurrent-edit orphan case already accepted below. **Known, deliberately unaddressed gap:** a CASCADE-deleted child row's own image field doesn't get cleaned up -- cascade deletes go through a raw bulk `DELETE`, not a recursive `GenericDao.delete` call, so this cleanup step never reaches them.
...
- **Content-addressing** — keying by `{table}/{record_id}/{field_name}/{filename}` (as inherited from the old `attachment` sketch) versus a content hash. Hash-keying would dedupe identical images and make the "which file does this row actually need" question trivial, at the cost of an extra lookup layer. Not resolved — current design keeps the simpler path-based key.
  ...
  **Real, confirmed correctness bug this caused, fixed 2026-09-03 -- not a hypothetical this open item was flagging in the abstract.** The original `filename` was a fixed `image.<ext>` per field, so *every* recapture wrote the identical relative key. Caught on real hardware: MIKE-12R still had an old cached local file from earlier testing; a fresh photo captured on MIKE-CU, uploaded under that same unchanged key, never displaced it there -- `FileSyncService.fetch`'s "local file already exists -> skip the network" check has no way to know the hub's content changed if the key it's asked for never does. **Fixed by giving every capture a timestamped filename** (`image_<millis>.<ext>`) instead of a fixed one -- a genuinely new relative key per capture, so a device that's never seen it has no local file to short-circuit on. Not full content-addressing (no dedup, no hash), but it closes the specific hole that was actually hit: two different images at the same table/record/field can no longer collide on one key. `GenericFormScreen._ingestPickedImage` also now cleans up the *previous* key's hub copy on a replace (cheap, now that `FileSyncService.delete` already exists) -- otherwise every recapture would leave a permanent hub orphan, not just the already-accepted concurrent-edit case below.
