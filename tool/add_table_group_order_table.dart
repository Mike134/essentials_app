...
// One-time bootstrap that adds the new `table_group_order` infra table to
// an EXISTING, already-live essentials.db -- sidebar *group* display order
// (not table-within-a-group order, which already lives in `table_group
// .group_position`). Same chicken-and-egg reasoning as every other infra
// table's own original bootstrap (CLAUDE.md, Essentials v2 Phase 1/3/7):
// the migration_log/MigrationService pipeline that self-applies every
// *other* schema change can't create the table it needs to record what to
...
//
//   dart run tool/add_table_group_order_table.dart --path C:\Databases\essentials_app\essentials.db
//
// Idempotent: does nothing (prints and exits 0) if table_group_order
...
Future<void> main(List<String> args) async {
  ...
  // Same refusal as add_view_definitions_table.dart/add_template_definitions
  // _table.dart's own -- see those files' doc comments for the full
  // reasoning: a wrong device_id (e.g. this machine's own hostname applied
  // against hub.db) poisons migration_status under the wrong peer identity,
  // and re-running this against an already-bootstrapped db authors a
  // brand-new, colliding migration_log row every time.
  ...
  try {
    final existing = await crdt.query(
      "SELECT 1 FROM sqlite_master WHERE type = 'table' AND name = 'table_group_order'",
    );
    if (existing.isNotEmpty) {
      print('table_group_order already exists -- nothing to do.');
      return;
    }
    ...
    await crdt.transaction((txn) async {
      await txn.execute(
        ...
        [_ddl, 'Create table "table_group_order" (sidebar group display order)', createdAt],
      );
    });
    ...
    final columns = await crdt.query('PRAGMA table_info("table_group_order")');
    ...
    if (missing.isEmpty) {
      print('OK: table_group_order created (${columnNames.length} columns, CRDT columns present).');
      ...
      print('FAIL: table_group_order missing CRDT columns: $missing');
      exitCode = 1;
    }
  } finally {
    await crdt.close();
